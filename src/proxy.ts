...
export async function proxy(request: NextRequest) {
  ...
  // Expose the request URL to server components, read back through
  // `~/lib/routes/requestPath`. A layout can't read its child segments' params
  // (e.g. the active `journalpost`), so `saksmappe/[saksmappe]/layout.tsx` uses
  // `x-pathname` to center the journalpost window on a deep link. Layouts also
  // never receive `searchParams` at all, which is why `x-search` exists:
  // `@header/layout.tsx` owns the search field and needs the `enhet` filter to
  // seed its enhet cache. Built from `request.headers` so the cookie updates
