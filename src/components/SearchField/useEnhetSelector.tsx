...
type UseEnhetSelectorProps = {
  expanded?: boolean;
  close?: () => void;
};

export function useEnhetSelector({
  expanded = false,
  close,
}: UseEnhetSelectorProps = {}) {
  ...
  useEffect(() => {
    ...
    cachedTrimmedEnhetList().then((unfilteredEnhetList) => {
      ...
      // Remove parent.id where .parent doesn't exist in the list
  ...
  const selector: ReactNode = expanded ? (
    <EnhetSelector
      loading={loading}
      searchString={searchString}
      visibleEnhetNodeList={visibleEnhetNodeList}
      selectedEnhetList={selectedEnhetList}
      ...
      addEnhetHandler={addEnhetHandler}
      removeEnhetHandler={removeEnhetHandler}
    />
  ) : null;
...
function getScore(
  ...
  if (enhet.enhetstype !== 'DUMMYENHET') {
    ...
    const languageWeights = {
      nb: currentLanguageCode === 'nb' ? 1 : 0.1,
      nn: currentLanguageCode === 'nn' ? 1 : 0.1,
      se: currentLanguageCode === 'se' ? 1 : 0.1,
      en: currentLanguageCode === 'en' ? 1 : 0.1,
    };
    ...
    const bestScore = matches.reduce((currentScore, match) => {
      ...
      return Math.max(thisScore, currentScore);
    }, 0);
